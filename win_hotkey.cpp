...
const WCHAR *HOTKEYWIN_CLASSNAME = L"HotKeyWin";
LRESULT CALLBACK HotKeyWinProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case WM_HOTKEY:
        raw_key = (lParam & 0xFFFF0000) >> 16;
        modifier = lParam & 0xFFFF;
        notify((WId)hwnd, modifier, raw_key);
        break;
...
HWND createHotKeyWindow()
{
    HWND hwnd;
    hwnd = CreateWindowExW(
        0,                   /* Extended possibilites for variation */
        HOTKEYWIN_CLASSNAME, /* Classname */
        L"HotKeyWindow",      /* Title Text */
        ...
        NULL,                /* No menu */
        GetModuleHandle(nullptr),       /* Program Instance handler */
...
void registerWindowClass()
{
    ...
    if (RegisterClassExW(&wincl)) inited = true;
}


}

// see qt4.3.0/src/gui/kernel/qkeymapper_win.cpp
...
typedef QPair<unsigned int, unsigned int> WinKey; //Modifier & RawWinKey

                                                 /**
                                                 * 将Qt的按键转换成Windows按键值
                                                 * 参数hotkey Qt的按键序列，只取第一个
                                                 * 返回 修饰键与虚拟键
                                                 */
WinKey translateKey(QKeySequence hotkey)
{
    unsigned int key = hotkey[0];
    unsigned int raw_key = key & ~(Qt::CTRL | Qt::ALT | Qt::SHIFT);
    unsigned int raw_winkey = 0;
    ...
    if ((key & Qt::CTRL) != 0)	modifier |= MOD_CONTROL;
    if ((key & Qt::ALT) != 0) modifier |= MOD_ALT;
    if ((key & Qt::SHIFT) != 0) modifier |= MOD_SHIFT;

    return qMakePair(modifier, raw_winkey);
}

/**
* 用于管理当前所有热键管理器及相应的窗口
* XXX 据说静态变量的初始化顺序不是确定的，要写在函数里面。。
*/
QMap<HWND, WinHotKey *> *getDaemon()
{
    static QMap<HWND, WinHotKey *> hotKeyDaemons;
    return &hotKeyDaemons;
}

/**
* 被窗口回调函数调用，用于向热键管理器发送热键到达信息
*/
void notify(WId hwnd, unsigned int modifier, unsigned int raw_key)
{
    QMap<HWND, WinHotKey *>::iterator itor = getDaemon()->find((HWND)hwnd);
    if (itor != getDaemon()->end())
    {
        WinHotKey *whk = *itor;
        for (QMap<int, QKeySequence>::iterator itor2 = whk->hotkeys.begin(); itor2 != whk->hotkeys.end(); ++itor2)
        {
            WinKey wk = translateKey(*itor2);

            if (wk.first == modifier && wk.second == raw_key)
            {
                emit whk->catched(itor2.key());
                break;
            }
        }
    }
...
WinHotKey::WinHotKey(QObject *parent) :QObject(parent)
{
    registerWindowClass();
    //TODO 处理不能创建窗口的情况 
    hwnd = (WId)createHotKeyWindow();
    //ShowWindow(hwnd,SW_SHOW);
    lastid = 0;
    getDaemon()->insert((HWND)hwnd, this);
}

WinHotKey::~WinHotKey()
{
    for (QMap<int, QKeySequence>::iterator itor = hotkeys.begin(); itor != hotkeys.end(); ++itor)
    {
        UnregisterHotKey((HWND)hwnd, itor.key());
    }
    PostMessage((HWND)hwnd, WM_DESTROY, 0, 0);
    getDaemon()->remove((HWND)hwnd);
}

int WinHotKey::addHotKey(QKeySequence hotkey)
{
    for (QMap<int, QKeySequence>::iterator itor = hotkeys.begin(); itor != hotkeys.end(); ++itor)
    {
        if (*itor == hotkey) return -1;
    }

    unsigned int modifier, raw_winkey;
    WinKey wk = translateKey(hotkey);
    modifier = wk.first;
    raw_winkey = wk.second;

    if (!RegisterHotKey((HWND)hwnd, lastid, modifier, raw_winkey)) return -1;
    hotkeys.insert(lastid, hotkey);
    if (lastid >= 0xbfff) lastid = 0;
    return lastid++;
}

void WinHotKey::removeHotKey(int id)
{
    QMap<int, QKeySequence>::iterator itor = hotkeys.find(id);
    if (itor != hotkeys.end())
    {
